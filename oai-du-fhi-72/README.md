...
These charts are for benetel 550 (n7) deployed in Eurecom -3 Hallway (Communications department)
- TDD pattern: DDDSU
- Absolute frequency SSB: 663360
- DL Absolute frequency pointA: 660084
- Antenna configuration: 4T4R, 2/4 layer DL and 1 layer UL
- Bandwidth: 100MHz
- Band: n77
- PLMN 20899
- VLAN: 9 (Near Lab 312) 11 (End of the hallway)
...
1. Configure the baremetal DU server as mentioned in this [tutorial](https://gitlab.eurecom.fr/oai/openairinterface5g/-/blob/develop/doc/ORAN_FHI7.2_Tutorial.md?ref_type=heads#configure-your-server) 
...
For Ubuntu you can pull the image from oai docker-hub
...
|multus.f1Interface.hostInterface               |host interface                 |Host interface of the machine where pod will run|
|multus.ruInterface.create                      |true/false                     |(Optional not necessary)                        |
|multus.ruInterface.mtu                         |number                         |                                                |
|multus.ruInterface.vlan                        |numer                          |                                                |
|multus.ruInterface.cPlaneMacAdd                |Mac-address                    |                                                |
|multus.ruInterface.uPlaneMacAdd                |Mac-address                    |                                                |
|multus.ruInterface.sriovNetworkNamespace       |String                         |                                                |
|multus.ruInterface.sriovResourceNameCplane     |String                         |                                                |
|multus.ruInterface.sriovResourceNameUplane     |String                         |                                                |
...
The config parameters mentioned in `config` block of `values.yaml` are limited on purpose to maintain simplicity. They do not allow changing a lot of parameters of oai-du. If you want to use your own configuration file for oai-du, it is recommended to copy it in `templates/configmap.yaml`. The command line for du is provided in `config.useAdditionalOptions`. 
...
The charts are configured to be used with primary CNI of Kubernetes. To define static ip-addresses for f1 and RU, most of the primary CNIs do not allow static ip-address allocation. To overcome this we are using multus-cni with static ip-address allocation. 
...
1. Check the networking, config parameters of the file in `templates/configmap.yaml`. Once the du is configured.
